...
`
  ...
  `
    ...
    case "BOOKING_CONFIRMED_CLIENT":
      ...
      const confirmedText =
        language === "he"
          ? `שלום ${data.userName},\n\nההזמנה שלך ל${data.treatmentName} בתאריך ${new Date(data.bookingDateTime).toLocaleString("he-IL")} אושרה על ידי המטפל/ת ${data.professionalName}.\nתוכל/י לצפות בפרטי ההזמנה כאן: ${data.orderDetailsLink}\n\nבברכה,\nצוות ${emailFrom}`
          : language === "ru"
            ? `Здравствуйте, ${data.userName},\n\nВаш заказ на ${data.treatmentName} ${new Date(data.bookingDateTime).toLocaleString("ru-RU")} был подтвержден специалистом ${data.professionalName}.\nДетали заказа: ${data.orderDetailsLink}\n\nС уважением,\nКоманда ${emailFrom}`
            : `Hello ${data.userName},\n\nYour booking for ${data.treatmentName} on ${new Date(data.bookingDateTime).toLocaleString("en-US")} has been confirmed by ${data.professionalName}.\nView booking details: ${data.orderDetailsLink}\n\nRegards,\nThe ${emailFrom} Team`
      const confirmedHtml = `
      <p>${language === "he" ? `שלום ${data.userName},` : language === "ru" ? `Здравствуйте, ${data.userName},` : `Hello ${data.userName},`}</p>
      <p>${language === "he" ? `ההזמנה שלך ל<strong>${data.treatmentName}</strong> בתאריך <strong>${new Date(data.bookingDateTime).toLocaleString(language === "he" ? "he-IL" : language === "ru" ? "ru-RU" : "en-US")}</strong> אושרה על ידי המטפל/ת <strong>${data.professionalName}</strong>.` : language === "ru" ? `Ваш заказ на <strong>${data.treatmentName}</strong> ${new Date(data.bookingDateTime).toLocaleString(language === "he" ? "he-IL" : language === "ru" ? "ru-RU" : "en-US")} был подтвержден специалистом <strong>${data.professionalName}</strong>.` : `Your booking for <strong>${data.treatmentName}</strong> on <strong>${new Date(data.bookingDateTime).toLocaleString(language === "he" ? "he-IL" : language === "ru" ? "ru-RU" : "en-US")}</strong> has been confirmed by <strong>${data.professionalName}</strong>.`}</p>
