...
export default function GuestPurchaseSubscriptionClient({
  ...
  const [currentStep, setCurrentStep] = useState<
    "subscription" | "treatment" | "guest-info" | "summary" | "payment"
  >("subscription");
  ...
  const selectedSubscriptionData = useMemo(() => {
    return subscriptions.find(
      (sub) => sub._id.toString() === selectedSubscriptionId,
    );
  }, [subscriptions, selectedSubscriptionId]);

  const selectedTreatmentData = useMemo(() => {
    return treatments.find(
      (treatment) => treatment._id.toString() === selectedTreatmentId,
    );
  ...
  const calculatedPrice = useMemo(
    () => ({
      basePrice: totalSubscriptionPrice,
      surcharges: [],
      totalSurchargesAmount: 0,
      treatmentPriceAfterSubscriptionOrTreatmentVoucher: totalSubscriptionPrice,
      couponDiscount: 0,
      voucherAppliedAmount: 0,
      finalAmount: totalSubscriptionPrice,
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <p className="text-gray-600">
              פרטי המנוי נשלחו אליך באימייל. תוכל להשתמש במנוי להזמנת טיפולים.
            </p>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {subscriptions.map((subscription) => (
                <Card
                  ...
                  onClick={() =>
                    setSelectedSubscriptionId(subscription._id.toString())
                  }
                >
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                  ...
                  <CardContent>
                    ...
                    <span className="inline-block bg-green-100 text-green-800 px-2 py-1 rounded text-sm">
                      {subscription.quantity + subscription.bonusQuantity}{" "}
                      טיפולים
                    </span>
                  </CardContent>
                </Card>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {treatments.map((treatment) => (
                <Card
                  ...
                  onClick={() =>
                    setSelectedTreatmentId(treatment._id.toString())
                  }
                >
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
          ...
          <div className="space-y-6">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600 mt-2">
                בדוק את פרטי ההזמנה לפני האישור הסופי
              </p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <Card className="mb-4">
                  ...
                  <CardContent>
                    <p className="text-gray-600 mb-2">
                      {selectedSubscriptionData?.description}
                    </p>
                    <span className="inline-block bg-green-100 text-green-800 px-2 py-1 rounded text-sm">
                      {(selectedSubscriptionData?.quantity || 0) +
                        (selectedSubscriptionData?.bonusQuantity || 0)}{" "}
                      טיפולים
                    </span>
                  </CardContent>
                </Card>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex justify-between text-sm text-gray-600 mb-2">
          {steps.map((step, index) => (
            <span
              key={step.key}
              className={`${steps.findIndex((s) => s.key === currentStep) >= index ? "text-blue-600 font-medium" : ""}`}
            >
              {step.label}
            </span>
          ))}
      ...
      <div className="min-h-[400px] relative mb-8">{renderStep()}</div>
