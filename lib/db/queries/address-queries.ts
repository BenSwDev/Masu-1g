...
class AddressQueries {
  ...
  static async getUserAddresses(userId: string) {
    return await Address.find({ userId }).sort({ isDefault: -1, createdAt: -1 }).lean().exec()
  }

  // Get default address for a user
  static async getDefaultAddress(userId: string) {
    return await Address.findOne({ userId, isDefault: true }).lean().exec()
  }
  ...
  static async createAddress(data: Partial<IAddress>) {
    ...
    if (data.isDefault) {
      await Address.updateMany({ userId: data.userId }, { $set: { isDefault: false } }).exec()
    }
  ...
  static async updateAddress(id: string, userId: string, data: Partial<IAddress>) {
    ...
    if (data.isDefault) {
      await Address.updateMany({ userId, _id: { $ne: id } }, { $set: { isDefault: false } }).exec()
    }

    return await Address.findOneAndUpdate({ _id: id, userId }, { $set: data }, { new: true }).exec()
  }
  ...
  static async setDefaultAddress(id: string, userId: string) {
    ...
    await Address.updateMany({ userId }, { $set: { isDefault: false } }).exec()
