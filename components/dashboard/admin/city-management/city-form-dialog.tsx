    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Input
            name="lat"
            defaultValue={city?.coordinates ? String(city.coordinates.lat) : ""}
            placeholder={t("admin.cities.form.lat") as string}
            aria-label={t("admin.cities.form.lat") as string}
          />
          <Input
            name="lng"
            defaultValue={city?.coordinates ? String(city.coordinates.lng) : ""}
            placeholder={t("admin.cities.form.lng") as string}
            aria-label={t("admin.cities.form.lng") as string}
          />
          <div className="flex items-center space-x-2">
            <Checkbox id="isActive" name="isActive" value="true" defaultChecked={city ? city.isActive : true} />
