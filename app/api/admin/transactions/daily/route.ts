  ...
  try {
    ...
    for (const booking of bookings) {
      const customer = booking.userId
        ? await db.collection("users").findOne({ _id: booking.userId })
        : null
      const finalAmount = booking.priceDetails?.finalAmount || booking.totalAmount || 0
    ...
    for (const voucher of newVouchers) {
      const customer = voucher.purchaserUserId
        ? await db.collection("users").findOne({ _id: voucher.purchaserUserId })
        : null
    ...
    for (const booking of voucherBookings) {
      ...
      transactions.push({
        ...
        status: "redeemed",
        transactionId:
          booking.priceDetails?.appliedGiftVoucherId?.toString() || booking._id.toString(),
      })
    ...
    for (const subscription of newSubscriptions) {
      const customer = subscription.userId
        ? await db.collection("users").findOne({ _id: subscription.userId })
        : null
      const subscriptionDetails = await db
        .collection("subscriptions")
        .findOne({ _id: subscription.subscriptionId })
    ...
    for (const booking of subscriptionBookings) {
      ...
      transactions.push({
        ...
        status: "redeemed",
        transactionId:
          booking.priceDetails?.redeemedUserSubscriptionId?.toString() || booking._id.toString(),
      })
