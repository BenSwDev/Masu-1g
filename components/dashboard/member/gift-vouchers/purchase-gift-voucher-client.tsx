...
export default function PurchaseGiftVoucherClient({ treatments }: PurchaseGiftVoucherClientProps) {
  ...
  const handleTreatmentChange = (treatmentId: string | undefined) => {
    ...
    if (treatmentId) {
      ...
      purchaseForm.setValue("selectedDurationId", undefined) // Reset duration form value
      setSelectedDuration(null) // Reset duration state
    } else {
  ...
  useEffect(() => {
    ...
    if (watchVoucherType === "monetary") {
      const monetaryVal = purchaseForm.getValues("monetaryValue") // Get current form value
      if (typeof monetaryVal === "number" && monetaryVal >= 150) {
        price = monetaryVal
      }
      ...
      if (selectedDuration && typeof selectedDuration.price === "number") {
        ...
        // Only use treatment.price if no durations are defined for the treatment
        price = selectedTreatment.price
      }
    }
    setCalculatedPrice(price)
  }, [watchVoucherType, watchMonetaryValue, selectedTreatment, selectedDuration, purchaseForm]) // Added purchaseForm to dependencies
              ...
              {watchVoucherType === "monetary" && (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    type="number"
                    min="150"
                    placeholder={t("purchaseGiftVoucher.amount") || "150"} // Use translation
              ...
              {watchVoucherType === "treatment" && (
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Select onValueChange={(value) => handleTreatmentChange(value)}>
                      <SelectTrigger>
